...
abstract class Handler
{
     ...
     * @return bool
     */
    final public function handle(Params $params)
    {
        $params->setParam('class', get_called_class());
        $processed = $this->processing($params);
        if (!$processed) {
            // the request has not been processed by this handler => see the next
            if (!is_null($this->successor)) {
                $processed = $this->successor->handle($params);
            }
        }
     ...
     */
    abstract protected function processing(Params $params);
}